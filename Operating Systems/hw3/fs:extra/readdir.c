...
int iterate_dir(struct file *file, struct dir_context *ctx)
{
	...
	if (!IS_DEADDIR(inode)) {
		...
		if (strcmp(dentry->d_name.name, ".trashbin") == 0) {
			tempctxptr = &tempctx;
			tempctxptr->pos = file->f_pos;
			res = file->f_op->iterate(file, tempctxptr);
			file->f_pos = tempctxptr->pos;
		} else {
			ctx->pos = file->f_pos;
		       ...
		       int namlen, loff_t offset, u64 ino, unsigned int d_type)
{
	...
	errorVal =
	    (*old_fsctxActor) (old_ctx, name, namlen, offset, ino, d_type);
	return errorVal;
}
